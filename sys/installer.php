<?php
function check_mime(){
	// Nachschauen ob gsheet etc. bereits in der MIME-Datei hinterlegt ist
	$mime_file 		= file(__DIR__ .'/../../../lib/private/mimetypes.list.php');
	$search 		= 'gsheet'; //check for all
	...
	if(!count($matches) > 0){ //Nicht gefunden, wird hinzugefuegt
		...
		for($i = 0; $i < count($mime_file)+1; $i++){
			...
			if($i == $key[0]+1){
				$new[$i] 	 = "	'gslides' => array('application/gslides', null),\r\n";
				$new[$i] 	.= "	'gsheet' => array('application/gsheet', null),\r\n";
				$new[$i] 	.= "	'gdoc' => array('application/gdoc', null),";
				$new_passed = 1;
			} else {
...
if(!file_exists(__DIR__ . '/installed')){
	if(check_mime() == 1){
		//Refresh filecache after adding gsheet etc. to MIME
